...
class FakeNameGenerator():
    ...
    def GenerateIdenity(self):
        ...
        return self.addy,self.SSN,self.phone,self.phoneprefix,self.birthday,self.age,self.tropicalzodiac,self.email,self.username,self.password,self.website,self.useragent,self.cardtype,self.card,self.exp,self.CVC,self.company,self.job,self.height,self.weight,self.bloodtype,self.UPSTrackingnum,self.MoneyGram,self.WesternUnion,self.favcolor,self.car,self.GUID
